                 ...
                 {filteredPatients.map(p => {
                    ...
                    return (
                       <tr key={p.id} className="hover:bg-muted/30 transition-colors cursor-pointer">
                          ...
                          <td className="p-4">
                             {completed > 0 ? (
                               <span className="text-xs font-medium text-emerald-600 bg-emerald-100 px-2 py-1 rounded-md">Returning</span>
                             ) : (
                               <span className="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-1 rounded-md">New Patient</span>
                             )}
                          </td>
                       </tr>
                    )
                 ...
                 {filteredPatients.length === 0 && (
                    <tr>
                       <td colSpan={5} className="p-8 text-center text-muted-foreground">
                          {allPatients.length === 0 ? "No patients have booked appointments yet." : "No matching patients found."}
                       </td>
                    </tr>
                 )}
              </tbody>
