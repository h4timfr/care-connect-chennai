-- 00011_security_correction.sql
-- 1. Harden check_appointment_update() with STRICT restrictions on patient/staff updates
...
BEGIN
    ...
    ELSIF v_is_staff THEN
        -- Staff cannot change clinic, patient, fee, or doctor
        IF NEW.clinic_id != OLD.clinic_id OR
           NEW.patient_id != OLD.patient_id OR
           NEW.fee != OLD.fee OR
           NEW.doctor_id != OLD.doctor_id THEN
            RAISE EXCEPTION 'Staff cannot reassign appointments to other clinics, patients, doctors, or change fees';
        END IF;
...
-- Secure EXECUTE privileges on the trigger function
